...
>
> **[인계: M4]** 보스 전용 스테이지 레이아웃(엘리트 전용 투기장: 봄 숲 / 심해 / 가을 미로 / 겨울 성벽) — 엘리트 4종은 절차 생성 아닌 고정 레이아웃이므로 SO/씬으로 정의 필요.
...
- **스테이지 세로 = 카메라 시야 세로 × 3** (`orthographicSize × 2 × 3`, ±0.5 Unit). SegmentLayoutBuilder가 자동 추종
...
>
> **[인계: M4]** 보스/엘리트 격파 보상 지급 (`OnBossDefeated` 이벤트 페이로드: 골드/마나 결정/보스의 영혼/SP/엘리트 고유 코어 조각·전설 룬). `EconomyManager` 도입 시 OnBossDefeated 구독자 추가로 자동 지급. 황금 고블린 왕의 황금 폭탄 골드 드랍도 함께. 보스/엘리트 자힐·재생 메커니즘(세계수 광합성/크라켄 촉수 재생/리바이어선 자힐/겨울 여왕 빙결 재생) — `MonsterBase`에 `Heal(int)` public API 추가 후 정식 구현 (마일스톤 4에서는 시각 표시만). 보스 도감 / 시련 도감(점성술사 시설). 보스 연습 모드(수련장 시설). 엘리트 입장 조건 검증(해당 액트 최종 보스 처치 여부, 주점 의뢰 시스템).
